...
def parse_seeds_file(seeds_file):
    ...
    with open(seeds_file, 'r') as f:
        ...
        for line in f:
            ...
            # Assign a dummy score=1.0 for bridging logic (unused here, but kept for consistency)
            score = 1.0
...
def create_graph_instance(networks_dir, diamond=False):
    """
    Build a NetworkX graph using the edge_scores.txt (or edge_diamond_file.txt if diamond=True).
    """
...
def decide_longest_connected_component(components, node_to_score):
    ...
    (Not used when no bridging is required.)
    """
    if not components:
        return set(), []
    max_len = 0
...
def create_subnetwork_from_seeds_no_scores(
    ...
    We parse the seeds, assigning *score=1.0* to each.
    Then we build the subnetwork using only the edges connecting seeds.
    ...
    # Build node->score dictionary (unused here, but kept for consistency)
    node_to_score = {}
    ...
    # (Optional) Check for precomputed shortest paths file – not used here since bridging is disabled.
    all_shortest_paths_file = os.path.join(networks_dir, "all_shortest_paths.txt")
    if not os.path.exists(all_shortest_paths_file):
        print("all_shortest_paths.txt not found; not used in this script since bridging is disabled.")
    else:
        print("Using existing shortest paths file (not used in this script):", all_shortest_paths_file)

    # 3) Build the subnetwork of seeds using only seed-to-seed edges
    subG = nx.Graph()
    edges = []

    # Parse edge_scores.txt => keep edge if both ends are in seeds_dict
    edge_scores_file = os.path.join(networks_dir, "edge_scores.txt")
    ...
    with open(edge_scores_file, 'r') as ef:
        for line in ef:
            ...
            if len(parts) != 3:
                continue
            n1, w, n2 = parts
            if n1 in seeds_dict and n2 in seeds_dict:
                subG.add_node(n1)
                subG.add_node(n2)
                subG.add_edge(n1, n2)
                edges.append((n1, n2))

    # 4) (No bridging is performed, so we do not attempt to connect disconnected components.)

    # 5) Write subnetwork.sif
    subnetwork_file = os.path.join(output_dir, "subnetwork_genes.sif")
    with open(subnetwork_file, 'w') as sf:
        for (a, b) in edges:
            sf.write("%s interaction %s\n" % (a, b))
    
    # 6) Build JSON (only for seed nodes)
    rows = []
    for ue_id, vals in seeds_dict.items():
        uniprot = vals[0]
        gene    = vals[1]
        ...
        # Set visual properties for the seed nodes
        if tnode == 'seed':
    ...
    json_out_file = os.path.join(output_dir, network_json_file)
    with open(json_out_file, 'w') as jf:
        jf.write("[\n%s,\n%s\n]" % (node_str, edge_str))

    # 7) Plot the subnetwork using Matplotlib
    ...
    print("Cytoscape.js JSON written to:", json_out_file)
...
if __name__ == "__main__":
    ...
    network_json_file  = "subnetwork_genes.json"
    networks_dir       = "/home/patricia/Astrocytes_project/BIANA_phy"
    output_dir         = "."
    species            = "9606"
    diamond            = False
